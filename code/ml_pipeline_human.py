# -*- coding: utf-8 -*-
"""
人类 TE 预测 ML Pipeline
输入：human_full_dataset.csv
输出：model_results.txt + 2 张图
...
# ==================== 1. 读取数据 ====================
# 修改日期：2026-05-31
df = pd.read_csv("output1/human_full_dataset.csv")

# 排除无关列和全 NaN 列
drop_cols = ['gene_id', 'gene_name', 'GENE_ID', 'GENE_SYMBOL', 'DATASET_ID',
             'CHROMOSOME', 'TISSUECELLTYPE', 'CELL_LINE', 'CONDITION',
             'TR', 'EVI']
...
# 二值特征（不标准化）
binary_features = ['has_antisense_alu', 'has_3utr_alu']
...
print(f"训练集基因数: {df_full['gene_id'].iloc[train_idx].nunique()}")
print(f"测试集基因数: {df_full['gene_id'].iloc[test_idx].nunique()}")
...
def cv_evaluate(name, model, X_tr, y_tr, groups_tr):
    ...
    for train_i, val_i in kf.split(X_tr, y_tr, groups_tr):
        X_tr_fold, X_val_fold = X_tr.iloc[train_i], X_tr.iloc[val_i]
        y_tr_fold, y_val_fold = y_tr[train_i], y_tr[val_i]
        m = model.__class__(**model.get_params())
        m.fit(X_tr_fold, y_tr_fold)
        y_val_pred = m.predict(X_val_fold)
        cv_r2.append(r2_score(y_val_fold, y_val_pred))
        cv_r.append(pearsonr(y_val_fold, y_val_pred)[0])
...
# 标注 intronic_alu_count 排位
intronic_rank = feat_imp['feature'].tolist().index('intronic_alu_count') + 1
total_feats = len(feat_imp)
print(f"\n===== 特征重要性 =====")
print(f"内含子 Alu 排位: 第 {intronic_rank}/{total_feats}")
...
# (4) 特征重要性横柱图
ax = axes[1, 1]
colors = ['coral' if f == 'intronic_alu_count' else 'steelblue' for f in feat_imp['feature']]
ax.barh(range(len(feat_imp)), feat_imp['importance'], color=colors)
ax.set_yticks(range(len(feat_imp)))
ax.set_yticklabels(feat_imp['feature'], fontsize=8)
ax.set_xlabel('Importance')
ax.set_title(f'Feature Importance (XGBoost)\nintronic_alu_count rank: #{intronic_rank}/{total_feats}')
# 标注
for i, (_, row) in enumerate(feat_imp.iterrows()):
    if row['feature'] == 'intronic_alu_count':
        ax.annotate(f'  #{intronic_rank}', xy=(row['importance'], i),
                    va='center', color='coral', fontweight='bold')

plt.tight_layout()
plt.savefig('human_ml_results.png', dpi=150)
print("\n图已保存: human_ml_results.png")

# ==================== 8. 输出结果到文件 ====================
# 修改日期：2026-05-31
with open("output1/model_results.txt", "w", encoding='utf-8') as f:
    f.write("===== Human TE Prediction Results =====\n")
    f.write(f"Total samples: {len(df)}, Features: {len(feature_cols)}\n")
    f.write(f"Train: {len(train_idx)}, Test: {len(test_idx)}\n\n")

    f.write("----- Baseline -----\n")
    f.write(f"Mean prediction R2 : {r2_mean:.4f}\n")
    f.write(f"Median prediction R2: {r2_median:.4f}\n\n")
    ...
    for name, yp, cv_r2_v, cv_r_v in [
        ...
        f.write(f"CV Pearson r  = {cv_r_v:.4f}\n\n")
...
print("\n结果已保存: output1/model_results.txt")
print("图已保存: human_ml_results.png")