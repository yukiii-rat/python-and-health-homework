"""
============================================================================
5'UTR Alu binary classification -- upregulate vs downregulate TE
============================================================================
...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'output', 'ml_right', 'ml_5UTR_classify')
...
def main():
    print('=' * 60)
    print("5'UTR Alu binary classification (up/down TE)")
    print('=' * 60)

    # ---- 1. Load data ----
    print(f'\n[1] Loading 5\'UTR subset...')
    df = pd.read_csv(INPUT_CSV)
    df = df[df['region_inserted'].str.contains("5'")].copy()
    print(f'  N={len(df)}')
    ...
    if do_shap:
        ...
        for rank, i in enumerate(order[:6], 1):
            ...
            ax.set_title(f"5'UTR -- {var}  (r={corr:+.3f})", fontsize=11)
    ...
    lines = [
        '=' * 70,
        "  5'UTR Alu Binary Classification Results",
        '=' * 70,
    ...
    if do_shap:
        ...
        lines.append('')
        lines.append("  5'UTR classification AUC < 0.65 -- no predictive power")
    ...
    write_report(os.path.join(OUTPUT_DIR, 'utr5_classification.txt'), lines)
    print(f'  -> utr5_classification.txt')
    ...
    if do_shap:
        ...
        print(f"5'UTR classification AUC={mean_auc:.4f} < {AUC_THRESHOLD}, no predictive power")
    print(f'{"=" * 60}')
