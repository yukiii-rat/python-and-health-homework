...
输出：alu_exonic_mapping.csv

处理流程：
1. 从 GTF 筛选 canonical transcript（tag 为 MANE_Select 或 basic，每个 gene_id 只取一个）
2. 提取每个 canonical transcript 的 5'UTR、CDS、3'UTR 坐标
3. 将 Alu BED 与外显子坐标取交集
4. 边界过滤：外显子总重叠比例 ≥ EXONIC_OVERLAP_THRESHOLD
5. Alu 跨越多区域时按 CDS > 3UTR > 5UTR 优先级分配
...
# ==================== 参数设置 ====================
# 修改日期：2026-05-30
ALU_BED = "alu_hg38.bed"
GTF_FILE = "gencode.v49.primary_assembly.annotation.gtf"
OUTPUT_CSV = "alu_exonic_mapping.csv"
...
print(f"读取 Alu 数量: {len(alu_df)}")
print(f"Alu 列: {list(alu_df.columns)}")
...
# 临时存储所有 transcript 的 line 中提取的信息
...
with open(GTF_FILE, 'r', encoding='utf-8') as f:
    for line in f:
        ...
        if feature == 'gene':
            ...
            # 检查是否为 canonical（MANE_Select 或 basic tag）
            is_canonical = False
            tag_match = re.findall(r'tag\s+"([^"]*)"', attr_str)
            for tag in tag_match:
                if tag == 'MANE_Select' or tag == 'basic':
                    is_canonical = True
                    break
...
# ==================== 6. 计算 Alu 与每个区域的重叠 ====================
# 修改日期：2026-05-30
# 向量化计算重叠：对每对 Alu x 区域，检查染色体是否匹配，计算重叠长度
print("正在计算 Alu 与区域的重叠...")

results = []
# 按染色体分组加速
for chrom, alu_chrom in alu_df.groupby('chrom'):
    region_chrom = region_df[region_df['chrom'] == chrom]
    if region_chrom.empty:
        continue

    # 遍历该染色体上的每个 Alu
    for _, alu_row in alu_chrom.iterrows():
        alu_id = alu_row['alu_id']
        alu_start = alu_row['start']
        alu_end = alu_row['end']
        alu_len = alu_row['length']
        alu_strand = alu_row['strand']

        # 找到与该 Alu 重叠的区域
        overlaps = region_chrom[
            (region_chrom['start'] <= alu_end) &
            (region_chrom['end'] >= alu_start)
        ]
        ...
        for _, reg_row in overlaps.iterrows():
            overlap_start = max(alu_start, reg_row['start'])
            overlap_end = min(alu_end, reg_row['end'])
            overlap_len = overlap_end - overlap_start + 1
            overlap_frac = overlap_len / alu_len
...
if not results:
    ...
    empty_df = pd.DataFrame(columns=[
        'alu_id', 'gene_id', 'gene_name', 'transcript_id',
...
# ==================== 8. 跨区域优先级分配 ====================
# 修改日期：2026-05-30
# Alu 跨越多区域时按 CDS > 3UTR > 5UTR 优先级分配
region_priority = {'CDS': 0, '3UTR': 1, '5UTR': 2}
filtered_df['priority'] = filtered_df['region'].map(region_priority)

# 对每个 (alu_id, gene_id, transcript_id) 取优先级最高的区域
# 如果有多个优先级相同的（如同一 alu 在一个 gene 内跨两个 CDS 外显子），保留 overlap_fraction 最大的
filtered_df = filtered_df.sort_values(
    ['alu_id', 'gene_id', 'transcript_id', 'priority', 'overlap_fraction'],
    ascending=[True, True, True, True, False]
).drop_duplicates(
    subset=['alu_id', 'gene_id', 'transcript_id'],
    keep='first'
)
...
# ==================== 9. 判断方向 ====================
# 修改日期：2026-05-30
# 将 Alu BED 的 strand 合并进来
alu_strand_map = alu_df.set_index('alu_id')['strand'].to_dict()
filtered_df['alu_strand'] = filtered_df['alu_id'].map(alu_strand_map)
filtered_df['orientation'] = np.where(
    filtered_df['alu_strand'] == filtered_df['strand'],
    'sense',
    'antisense'
)
